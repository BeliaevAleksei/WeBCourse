					...
					&& (!empty($_POST['Status']))&& (!empty($_POST['Pulpit'])) && (!empty($_POST['Teacher']))) 
				{
					...
					if(!$sql){
						...
						if(strcmp($course,1) == 0 || strcmp($course,2) == 0){
							$dao2->addStudent($firstName, $lastName, $course, "Haven't teacher", '', '');
						}
						if(strcmp($course,3) == 0 || strcmp($course,4) == 0){
							if(strcmp($status,"Haven't teacher") == 0 || strcmp($status,"Transfer") == 0)
							{
								$dao2->addStudent($firstName, $lastName, $course, $status, '', '');
							}
							else{
								$dao2->addStudent($firstName, $lastName, $course, $status, $teacher, $pulpit);
							}
						}
						echo "</br><p> Студент $firstName успешно добавлен </p></br>";
					}
				}
				else {
